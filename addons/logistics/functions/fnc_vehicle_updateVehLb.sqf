...
/*
Function: mti_logistics_fnc_vehicle_updateVehLb
...
Example:
    (begin example)
        [] call mti_logistics_fnc_vehicle_updateVehLb;
    (end)
...
private _allVehicles = "true" configClasses (missionConfigFile >> QGVAR(Config) >> "Vehicles");
_allVehicles append ("true" configClasses (configFile >> QGVAR(Config) >> "Vehicles"));
...

{
    ...
    private _limit = ["Vehicles",configName _x,"limit"] call FUNC(getConfigProperty);
