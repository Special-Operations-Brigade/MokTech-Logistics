...
/*
Function: mti_logistics_fnc_vehicle_onLoad

Description:
    Handles setting up the display, populating it with data and getting everything ready when the vehicle logibox menu is opened.
...
_logibox setVariable [QGVAR(fnc_dialogPFH),QFUNC(vehicle_dialogPFH)];
...
[_display,_logibox] call FUNC(vehicle_updateVehLb);

_listBoxVehicle lbSetCurSel 0;

// update the remaining elements based on currently selected vehicle
[_listBoxVehicle, 0] call FUNC(vehicle_updateDialog);

// add vehicle list box EH
_listBoxVehicle ctrlAddEventHandler ["LBSelChanged", { _this call FUNC(vehicle_updateDialog); }];
