...
/*
Function: mti_logistics_fnc_vehicle_updateDialog
...
// set limit text
private _limit = ["Vehicles",_curVehicle,"limit"] call FUNC(getConfigProperty);
...
lbClear _listBoxSkin;
private _hasSkins = GET_NUMBER(_curVehicleCfg >> QEGVAR(vehicles,hasSkins),0) > 0;

if (_hasSkins) then {
    private _skins = QUOTE(GET_NUMBER(_x >> QQEGVAR(vehicles,scope),0) > 0) configClasses (_curVehicleCfg >> "TextureSources");
